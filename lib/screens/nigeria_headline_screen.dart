...
class NigeriaHeadlineScreen extends ConsumerStatefulWidget {
  const NigeriaHeadlineScreen({Key? key}) : super(key: key);

  @override
  _NigeriaHeadlineScreenState createState() => _NigeriaHeadlineScreenState();
}

class _NigeriaHeadlineScreenState extends ConsumerState<NigeriaHeadlineScreen> {
  @override
  void initState() {
    super.initState();
    Future.microtask(() {
      ref.read(newsProvider.notifier).loadDiscoveryNews('nigeria_headlines');
    });
  }

  Future<void> _refreshNewsData() async {
    await ref.read(newsProvider.notifier).loadDiscoveryNews('nigeria_headlines');
  }
